...
class AutoComplete:
    """
    It works by buding a 'WordMap' 
    that stores words to word-follower-count
    ---------------------------------------
    e.g. To train the following statement:
    
    It is not enough to just know
    how tools work and what they worth,
    we have got to learn how to use them
    and to use them well.
    And with all these new weapons in your arsenal,
    we would better get those profits fired up
    ...
    For optimization,we use another store 'WordPrediction'
    to save the predictions for each word
    """
    ...
    def __init__(self):
        ...
        -Initialize database.we use sqlite3
         ...
         if not create them maintain a class level access 
         to the database connection object
         """
    ...
    def train(self, sentence):
        ...
        ---------------
        Trains the sentence.
        It does this by creating a map of
        current words to next words and their conts
        for each time the next word appears
        after the current word
        - takes in the sentence and splits it into a list of words
        - retrieves the word map and predictions map
        - create the word map and predictions map together
    ...
    def predict(self, word):
        ...
        """    
        cur = self.conn.cursor()
        predictions = cur.execute("SELECT value FROM WordPrediction WHERE name'predictions'"
        ).fetchone()[0]
        predictions = json.loads(predictions)
